    ...
    public class AccountController : ControllerBase
    {
        ...
        private async Task<OAuthResult> ProcessOAuthUser(string email, string? name, string externalId, string? avatar, string provider)
        {
            try
            {
                ...
                if (user == null)
                {
                    var validUsername = GenerateValidUsername(name ?? string.Empty, email);
                    user = new User
                    {
        ...
        private string GenerateValidUsername(string name, string email)
        {
            ...
            if (string.IsNullOrEmpty(cleanName) || cleanName.Length < 3)
            {
                ...
                cleanName = new string(cleanName.Where(c => char.IsLetterOrDigit(c)).ToArray());
            }
